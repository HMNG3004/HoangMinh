import { BlogsListViewModel } from "../models/Blogs";

export const blogs: BlogsListViewModel[] = [
  {
    title: "Chill cuối tuần với đồ án tốt nghiệp",
    slug: "chill-cuoi-tuan-voi-do-an-tot-nghiep",
    time: new Date(2024, 11, 24),
    tag: "Study",
  },
  {
    title: "Ôn tập kiến thức phần 3: C#",
    slug: "CSharp-interview-question",
    time: new Date(2024, 11, 21),
    tag: "Study",
  },
  {
    title: "Càng lớn, tớ càng thích những người dịu dàng",
    slug: "cang-lon-to-cang-thich-nhung-nguoi-diu-dang",
    time: new Date(2024, 11, 21),
    tag: "Daily",
  },
  {
    title: "Ôn tập kiến thức phần 2: Backend",
    slug: "backend-question-interview",
    time: new Date(2024, 11, 15),
    tag: "Study",
  },
  // {
  //   title: "Hôm nay mình hơi suy tư",
  //   slug: "hom-nay-minh-hoi-suy-tu",
  //   time: new Date(2024, 11, 2),
  //   tag: "Daily",
  // },
  {
    title: "Series ôn tập kiến thức - P1",
    slug: "series-on-tap-kien-thuc-p1",
    time: new Date(2024, 11, 15),
    tag: "Study",
  },
  {
    title: "Người giỏi sẽ trả giá bằng sự cô đơn",
    slug: "nguoi-gioi-se-tra-gia-bang-su-co-don",
    time: new Date(2024, 10, 16),
    tag: "Daily",
  },
  {
    title: "Nhật ký làm nô lệ cho đồ án tốt nghiệp",
    slug: "nhat-ky-lam-no-le-cho-do-an-tot-nghiep",
    time: new Date(2024, 10, 4),
    tag: "Daily",
  },
  {
    title: "Giới thiệu về mình",
    slug: "bai-viet-dau-tien",
    time: new Date(2024, 10, 1),
    tag: "Introduce",
  },
];
